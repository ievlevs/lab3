...
{
	this->m_currentPc = this->m_pc;
	if (this->m_currentPc % 4 != 0) {
		this->exception(EXCEPTION_LOAD_ADDRESS_ERROR);
	}
	Instruction instruction = Instruction(this->load32(this->m_currentPc));

	if (m_nextInstruction.getInstructionStatus() == instruction.INSTRUCTION_STATUS_UNALIGNED_ACCESS ||
		m_nextInstruction.getInstructionStatus() == instruction.INSTRUCTION_STATUS_UNHANDLED_FETCH) {
		return INSTRUCTION_TYPE_UNKNOWN;
	}
	this->m_pc = this->m_nextPc;
	this->m_nextPc = this->m_nextPc + 4;

	this->m_delaySlot = this->m_branch;
	this->m_branch = false;

	if (m_nextInstruction.getInstructionStatus() == instruction.INSTRUCTION_STATUS_NOT_IMPLEMENTED) {
		return INSTRUCTION_TYPE_NOT_IMPLEMENTED;
	}
	j += 1;
	RegisterData reg_data = this->m_load;
	this->setRegisterValue(reg_data.m_registerIndex, reg_data.m_registerValue);
	this->m_load = RegisterData(RegisterIndex(0), 0);
	Cpu::InstructionType res_instruction  = this->decodeAndExecute(instruction);
...
{
	uint32_t shift = instruction.getShiftImmediateValue();
	...
	uint32_t v = this->getRegisterValue(target_index) << shift;
...
Cpu::InstructionType Cpu::opcodeADDIU(const Instruction& instruction)
{
	...
	uint32_t value = this->getRegisterValue(source_register) + immediate_value;
...
{
	uint32_t jump_target = instruction.getJumpTargetValue();
	this->m_nextPc = (this->m_pc & 0xf0000000) | (jump_target << 2);
	this->m_branch = true;
...
Cpu::InstructionType Cpu::opcodeAND(const Instruction& instruction)
{
	...
	RegisterIndex source_register = instruction.getRegisterSourceIndex();
	RegisterIndex target_register = instruction.getRegisterTargetIndex();
	uint32_t value = (this->getRegisterValue(source_register) & this->getRegisterValue(target_register));
	this->setRegisterValue(dest_index, value);
...
{
	RegisterIndex cpuTargetRegister = instruction.getRegisterTargetIndex();
	uint32_t copDestRegister = instruction.getRegisterDestinationIndex().getRegisterIndex();
	uint32_t v;
	switch (copDestRegister) {
	case 12:
		v = this->m_sr;
		break;
	case 13:
	...
	default:
		...
		LOG("Unhandled read from cop0r{}" << std::hex << copDestRegister);
		return INSTRUCTION_TYPE_UNKNOWN;
	}
	this->m_load = Cpu::RegisterData(cpuTargetRegister, v);
...
Cpu::InstructionType Cpu::opcodeBNE(const Instruction& instruction)
{
	...
	RegisterIndex source_register = instruction.getRegisterSourceIndex();
	RegisterIndex target_register = instruction.getRegisterTargetIndex();
	if (this->getRegisterValue(source_register) != this->getRegisterValue(target_register)) {
		branch(immediate_value);
	}
...
{
	if ((this->m_sr & 0x10000) !=0) {
		LOG("Ignoring load while cache is isolated");
		return INSTRUCTION_TYPE_CACHE_ISOLATED;
	}
	uint32_t imm_se = instruction.getSignExtendedImmediateValue();
	RegisterIndex target_reg = instruction.getRegisterTargetIndex();
	RegisterIndex source_reg = instruction.getRegisterSourceIndex();
	
	uint32_t addr = this->getRegisterValue(source_reg) + imm_se;
	if (addr % 4 == 0) {
		Instruction value = this->load32(addr);
		uint32_t status = value.getInstructionStatus();

		if (status == Instruction::INSTRUCTION_STATUS_UNALIGNED_ACCESS ||
			status == Instruction::INSTRUCTION_STATUS_UNHANDLED_FETCH) {
			return INSTRUCTION_TYPE_UNKNOWN;
		}
		if (status == Instruction::INSTRUCTION_STATUS_NOT_IMPLEMENTED) {
			return INSTRUCTION_TYPE_NOT_IMPLEMENTED;
		}
		this->m_load = RegisterData(target_reg, value.getInstructionOpcode());

	}
		else {
			this->exception(Exception::EXCEPTION_LOAD_ADDRESS_ERROR);
		}
...
Cpu::InstructionType Cpu::opcodeSH(const Instruction& instruction)
{
	...
	uint32_t addr = this->getRegisterValue(source_reg) + imm_se;
	if (addr % 2 == 0) {
		uint32_t value = this->getRegisterValue(target_reg);
		this->store16(addr, value);
	}
	else {
		this->exception(Exception::EXCEPTION_STORE_ADDRESS_ERROR);
	}
...
Cpu::InstructionType Cpu::opcodeSB(const Instruction& instruction)
{
	...
	RegisterIndex target_register = instruction.getRegisterTargetIndex();
	RegisterIndex source_register = instruction.getRegisterSourceIndex();
	uint32_t addr = this->getRegisterValue(source_register) + immediate_value;
	uint32_t value = this->getRegisterValue(target_register);
	this->store8(addr, (uint8_t)value);
...
Cpu::InstructionType Cpu::opcodeJR(const Instruction& instruction)
{
	...
	RegisterIndex source_reg = instruction.getRegisterSourceIndex();
//	RegisterIndex source_reg = RegisterIndex(31);
	this->m_nextPc = this->getRegisterValue(source_reg);
...
{
	uint32_t extended_immediate_value = instruction.getSignExtendedImmediateValue();
	RegisterIndex target_register = instruction.getRegisterTargetIndex();
	RegisterIndex source_register = instruction.getRegisterSourceIndex();
	uint32_t addr = this->getRegisterValue(source_register) + extended_immediate_value;
	Instruction value = this->load8(addr);
	Instruction::InstructionStatus status = value.getInstructionStatus();
	if (status == Instruction::InstructionStatus::INSTRUCTION_STATUS_UNALIGNED_ACCESS || status == Instruction::InstructionStatus::INSTRUCTION_STATUS_UNHANDLED_FETCH) {
		return INSTRUCTION_TYPE_UNKNOWN;
	}
	if (status == Instruction::InstructionStatus::INSTRUCTION_STATUS_NOT_IMPLEMENTED) {
		return INSTRUCTION_TYPE_NOT_IMPLEMENTED;
	}
	this->m_load = RegisterData(target_register, (uint32_t)value.getInstructionOpcode());
...
{
	uint32_t extended_immediate_value = instruction.getSignExtendedImmediateValue();
	RegisterIndex source_regiser = instruction.getRegisterSourceIndex();
	RegisterIndex target_register = instruction.getRegisterTargetIndex();
	if (this->getRegisterValue(source_regiser) == this->getRegisterValue(target_register)) {
		branch(extended_immediate_value);
	}
...
Cpu::InstructionType Cpu::opcodeBGTZ(const Instruction& instruction)
{
	uint32_t immediate_value = instruction.getSignExtendedImmediateValue();
	RegisterIndex source_register = instruction.getRegisterSourceIndex();
	uint32_t value = this->getRegisterValue(source_register);
	if ( value > 0 ) {
		this->branch(immediate_value);
	}
...
{
	RegisterIndex dest_index = instruction.getRegisterDestinationIndex();
	RegisterIndex source_index = instruction.getRegisterSourceIndex();
	uint32_t ra = this->m_nextPc;
	this->setRegisterValue(dest_index, ra);
	this->m_nextPc = this->getRegisterValue(source_index);
	this->m_branch = true;
...
{
	uint32_t extended_value = instruction.getSignExtendedImmediateValue();
	RegisterIndex source_register = instruction.getRegisterSourceIndex();
	RegisterIndex target_register = instruction.getRegisterTargetIndex();

	uint32_t value = this->getRegisterValue(source_register) < extended_value;
	this->setRegisterValue(target_register, value);
...
Cpu::InstructionType Cpu::opcodeSUBU(const Instruction& instruction)
{
	...
	uint32_t value = this->getRegisterValue(source_reg) - this->getRegisterValue(target_reg);
	this->setRegisterValue(dest_reg, value);
...
Cpu::InstructionType Cpu::opcodeSRA(const Instruction& instruction)
{
	...
	uint32_t value = this->getRegisterValue(target_reg) >> shift;
	this->setRegisterValue(dest_reg, value);
...
{
	RegisterIndex dest_reg = instruction.getRegisterDestinationIndex();
	uint32_t lo = this->m_lo;
	this->setRegisterValue(dest_reg, lo);
...
Cpu::InstructionType Cpu::opcodeSLTIU(const Instruction& instruction)
{
	...
	uint32_t value = this->getRegisterValue(source_reg) < imm_se;
	this->setRegisterValue(target_reg, value);
